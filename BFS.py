...
pygame.display.set_caption("Breadth First Search Planner")
...
def BFS(draw, grid):
    ...
    while found == False and resign == False:
        ...
        if len(queue) == 0:                             # length of the queue will be 0 only if there is no path from start to end.
            resign = True
            print("No possible path !!!", iterations)
        
        else:
            next = queue.popleft()                      # As this is BFS, it works as First In First Out(FIFO). so we will be removing 1st element of the queue.
            x = next[0]
            y = next[1]
            grid[x][y].make_closed()                    # marking that grid as red
            iterations += 1
            #draw()
                ...
                for i in range(len(movement)):           # This loop will check all the neighbour of current grid.
                    ...
                    if x2 >= 0 and x2 < 128 and y2 >= 0 and y2 < 128:   # check if the new grid position lies within the boundary
                        if grid[x2][y2].is_barrier() == False and grid[x2][y2].get_pos() not in visited: # check if its not an obstacle and is not visited before.
                            ...
                            action[x2][y2] = i                       # to keep track of the path.
...
def main(win, width):                                      # Main loop
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:                        # press space bar to start the search
